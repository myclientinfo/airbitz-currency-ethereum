...
class ABCTxLibEth {
  ...
  gethRequest (entity, paramObject, setId) {
  ...
  blockHeightInnerLoop () {
    ...
    let blockNumberPromise = gethRequest('blockNumber', [], 1);

    blockNumberPromise.then(response => response.json())
      .then(jsonObj => {
        let blockHeight = parseInt(jsonObj.result, 16);
  ...
  makeSpend (abcSpendInfo) {
    ...
    const prom = new Promise((resolve, reject) => {
      ...
      for (let n in spendTargets) {
        ...
        totalSpends[currencyCode] += spendTarget.amountWei
        outputs.push({
          currencyCode,
          address: spendTarget.publicAddress,
          amount: spendTarget.amountWei
        })
      }
